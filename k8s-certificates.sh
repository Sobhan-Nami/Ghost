...
MASTER_1=$(dig +short master1)
MASTER_2=$(dig +short master2)
WORKER_1=$(dig +short worker1)
WORKER_2=$(dig +short worker2)
...
# Worker1 cert details
WORKER_1_CERT=/var/lib/kubelet/worker1.crt
WORKER_1_KEY=/var/lib/kubelet/worker1.key

# Worker1 kubeconfig location
WORKER_1_KUBECONFIG=/var/lib/kubelet/kubeconfig

# Worker1 kubelet config location
WORKER_1_KUBELET=/var/lib/kubelet/kubelet-config.yaml

# Systemd worker1 kubelet location
SYSTEMD_WORKER_1_KUBELET=/etc/systemd/system/kubelet.service

# kube-proxy worker1 location
...
echo "This script will validate the certificates in master as well as worker1 nodes. Before proceeding, make sure you ssh into the respective node [ Master or Worker1 ] for certificate validation"
echo
...
echo "  4. Verify kubeconfigs and PKI on worker1 Node after step 10"
echo "  5. Verify kubeconfigs and PKI on worker2 Node after step 11"
echo
...
case $value in
  ...
  1)
    if ! [ "${HOST}" = "master1" -o "${HOST}" = "master2" ]
    then
        printf "${FAILED}Must run on master1 or master2${NC}\n"
        exit 1
    fi
  ...
  4)
    if ! [ "${HOST}" = "worker1" ]
    then
        printf "${FAILED}Must run on worker1${NC}\n"
        exit 1
    fi
    ...
    check_cert_only "ca" $SUBJ_CA $CERT_ISSUER
    check_cert_and_key "kube-proxy" $SUBJ_KP $CERT_ISSUER
    check_cert_and_key "worker1" "Subject:CN=system:node:worker1,O=system:nodes" $CERT_ISSUER
  ...
  5)
    if ! [ "${HOST}" = "worker2" ]
    then
        printf "${FAILED}Must run on worker2${NC}\n"
        exit 1
    fi
    ...
    check_cert_only "kubelet-client-current" "Subject:O=system:nodes,CN=system:node:worker2" $CERT_ISSUER
    check_kubeconfig "kube-proxy" "/var/lib/kube-proxy" "https://${LOADBALANCER}:6443"
    ;;
